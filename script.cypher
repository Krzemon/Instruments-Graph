...
// ===========================
// --- Akcje
MERGE (:Asset {id:"AAPL", name:"Apple Inc."});
MERGE (:Asset {id:"MSFT", name:"Microsoft Corp."});
MERGE (:Asset {id:"SPY", name:"S&P500 ETF"});
MERGE (:Asset {id:"EEM", name:"Emerging Markets ETF"});

// --- Obligacje
MERGE (:Asset {id:"UST10Y", name:"US Treasury 10Y"});
MERGE (:Asset {id:"LQD", name:"Corporate Bond ETF"});
MERGE (:Asset {id:"HYG", name:"High Yield Bond ETF"});
MERGE (:Asset {id:"IEGA", name:"EU Government Bond ETF"});

// --- Surowce
MERGE (:Asset {id:"XAU", name:"Gold"});
MERGE (:Asset {id:"XAG", name:"Silver"});
MERGE (:Asset {id:"OIL", name:"Brent Oil"});
MERGE (:Asset {id:"NG", name:"Natural Gas"});

// --- Kryptowaluty
MERGE (:Asset {id:"BTC", name:"Bitcoin"});
MERGE (:Asset {id:"ETH", name:"Ethereum"});
MERGE (:Asset {id:"BNB", name:"BNB"});
MERGE (:Asset {id:"SOL", name:"Solana"});

// --- Forex
MERGE (:Asset {id:"EURUSD", name:"EUR/USD"});
MERGE (:Asset {id:"GBPUSD", name:"GBP/USD"});
MERGE (:Asset {id:"USDJPY", name:"USD/JPY"});
MERGE (:Asset {id:"USDCAD", name:"USD/CAD"});
...
MATCH (a:Asset)
WHERE a.id IN ["UST10Y","LQD","HYG","IEGA"]
...
WHERE (a.id="AAPL" AND b.id="MSFT") OR (a.id="AAPL" AND b.id="SPY") OR (a.id="MSFT" AND b.id="SPY")
   ...
   OR (a.id="UST10Y" AND b.id="IEGA") OR (a.id="LQD" AND b.id="IEGA") OR (a.id="BTC" AND b.id="SOL")
...
CASE a.id + "-" + b.id
  ...
  WHEN "UST10Y-IEGA" THEN 0.80
  WHEN "LQD-IEGA" THEN 0.75
...
// Niskie
MATCH (a:Asset), (b:Asset)
WHERE (a.id="XAU" AND b.id="BTC") OR (a.id="UST10Y" AND b.id="BTC") OR (a.id="IEGA" AND b.id="OIL")
MERGE (a)-[:CORRELATED {value:
CASE a.id + "-" + b.id
  WHEN "XAU-BTC" THEN 0.15
  WHEN "UST10Y-BTC" THEN 0.20
  WHEN "IEGA-OIL" THEN 0.10
END
}]->(b);

...
// 1. AssetGroup
// ===========================
MERGE (:AssetGroup {name: "Ryzykowne"});
MERGE (:AssetGroup {name: "Bezpieczne"});
...
// ===========================
MATCH (equity:AssetClass {name:"Akcje"}), (gRisk:AssetGroup {name:"Ryzykowne"}) MERGE (equity)-[:SUBCLASS_OF]->(gRisk);
MATCH (crypto:AssetClass {name:"Kryptowaluty"}) MERGE (crypto)-[:SUBCLASS_OF]->(gRisk);
MATCH (commodities:AssetClass {name:"Surowce"}) MERGE (commodities)-[:SUBCLASS_OF]->(gRisk);
MATCH (bonds:AssetClass {name:"Obligacje"}), (gSafe:AssetGroup {name:"Bezpieczne"}) MERGE (bonds)-[:SUBCLASS_OF]->(gSafe);
MATCH (forex:AssetClass {name:"Forex"}) MERGE (forex)-[:SUBCLASS_OF]->(gSafe);
